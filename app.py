...
# 0) 기본 UI 설정 (춘천공장 버전)
# =========================================================
# [수정 포인트] 아래 제목을 춘천공장에 맞게 변경하세요!
st.set_page_config(page_title="춘천공장 위생 개선관리", layout="wide", initial_sidebar_state="collapsed")
SYSTEM_TITLE = "춘천공장 위생 개선관리" 
    ...
    if not raw_tasks:
        ...
        if total_cnt == 0: st.warning("데이터가 없습니다.")
            ...
            with c_g_table:
                ...
                st.dataframe(grade_stats.rename(columns={'grade': '등급'}), 
                             column_config={"등급": st.column_config.TextColumn("등급"), "발생건수": st.column_config.NumberColumn("발생", format="%d"), "완료건수": st.column_config.NumberColumn("완료", format="%d"), "개선율": st.column_config.ProgressColumn("진행률", format="%.1f%%", min_value=0, max_value=100)},
